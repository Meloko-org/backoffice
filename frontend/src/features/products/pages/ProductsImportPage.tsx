// import type {  ProductsImportState, } from "../types/productsImport.types";
import { useProductsImport } from "../../../hooks/useProductsImport";

import { ImportSummary } from "../components/Importsummary";
import { ImportErrors } from "../components/ImportErrors";
import { ImportWarnings } from "../components/ImportWarnings";
import { ImportPreview } from "../components/ImportPreview";

import ImportDropzone from "../components/ImportDropzone";
import { ImportActions } from "../components/ImportActions";
import { ImportSuccess } from "../components/ImportSuccess";
import { ImportProgressModal } from "../components/ImportProgressModal";


// const initialState: ProductsImportState = {
//   status: "idle",
//   file: null,
//   report: null,
//   error: null,
// };



export default function ProductsImportPage() {
  
  const { state, setMode, selectFile, runDryRun, confirmImport, reset } = useProductsImport();

  console.log("mode :", state.mode)

  return (
    <div className="p-6 space-y-6">
      {(state.status === "dryRun" || state.status === "importing") && (
        <ImportProgressModal status={state.status} />
      )}

      <h1 className="text-2xl font-semibold text-shadow-black">
        Import des produits
      </h1>

      <ImportDropzone 
        key={state.resetKey}
        onFileSelected={selectFile}
        error={state.error}
      />

      <div className="flex items-center gap-4 rounded-lg border p-4 bg-gray-50">
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-900">
            Mode d’import
          </p>
          <p className="text-xs text-gray-600">
            {state.mode === "strict"
              ? "Aucune erreur autorisée"
              : "Les lignes en erreur seront ignorées"}
          </p>
        </div>

        <label className="relative inline-flex items-center cursor-pointer">
          <input
            type="checkbox"
            className="sr-only peer"
            checked={state.mode === "permissive"}
            onChange={(e) =>
              setMode(e.target.checked ? "permissive" : "strict")
            }
          />
          <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-primary-600 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:h-5 after:w-5 after:rounded-full after:transition-all peer-checked:after:translate-x-full" />
        </label>
      </div>



      {state.status === "fileReady" && (
        <button
          onClick={runDryRun}
          className="px-4 py-2 rounded-lg bg-primary-600 text-white font-medium hover:bg-primary-700 transition"
        >
          Analyser le fichier (dry run)
        </button>
      )}

      {state.status === "dryRun" && (
        <p className="text-sm text-gray-600">
          Analyse du fichier en cours…
        </p>
      )}

      {state.status === "preview" && state.report && state.report.meta && (
        <div className="space-y-6">
          <ImportSummary
            report={state.report}
          />

          <ImportErrors
            mode={state.report.meta.mode}
            errors={state.report.errors}
          />

          <ImportWarnings warnings={state.report.warnings} />

          {state.status === "preview" && state.report?.preview && state.report?.preview.products && (
            <>
            {state.report.preview.ignoredRows > 0 && (
              <p className="text-sm text-gray-600">
                {state.report.preview.ignoredRows} lignes ignorées
              </p>
            )}
              <ImportPreview
                productsCount={state.report.preview.productsCount}
                products={state.report.preview.products}
              />
            </>
          )}

          <div className="flex justify-end gap-3">
            {/* <button className="px-4 py-2 rounded border">
              Nouveau test
            </button> */}

            <ImportActions
              mode={state.report.meta.mode}
              hasBlockingErrors={state.report.errors.length > 0}
              status={state.status}
              onConfirm={confirmImport}
              onReset={reset}
            />
          </div>
        </div>
      )}

      {state.status === "success" && state.report && (
        <ImportSuccess
          report={state.report}
          onReset={reset}
          onGoToProducts={() => {}}
          // onGoToProducts={() => navigate("/products")}
        />
      )}

    </div>
  );
}
